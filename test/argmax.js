...
function simpleTestCases(){
	...
	tape("argmax works with integers", function(t){
		t.plan(1);
		var frame = new Frame({
		...
		var expected = 6; // 1 + 2 + 2 + 3 + 1 + 3 + 4 + 2 + 1

		var actual = frame.argmax("value");
	...
	tape("argmax works with integers", function(t){
		...
		var expected = 10; // (4 + 2 + 7 + 1 + 3 + 6 + 5 + 2 + 1 + 7 + 8) / 11
	...
	tape("argmax works floats", function(t){
		t.plan(1);
		var frame = new Frame({
		...
		var expected = 3; // (1.2 + 6.4 + 2.3 + 12.1 + 1.6 + 3.5 + 7.2 + 2.1 + 10.2) / 9
		var actual = frame.argmax("value");
	...
	tape("argmax works floats", function(t){
		...
		var frame = new Frame({
			"id"  :   [0, 2, 0, 1, 1, 0, 2, 0, 1],
			"value" : [1.2, 6.4, 2.3, 1.1, 10.6, 3.5, 7.2, 2.1, 9.2]
		});

		var expected = 4; // (1.2 + 6.4 + 2.3 + 12.1 + 1.6 + 3.5 + 7.2 + 2.1 + 10.2) / 9
		var actual = frame.argmax("value");

		t.equal(actual, expected);
	});

	tape("argmax wonky edge case", function(t){
		t.plan(1);
		var frame = new Frame({
			"id"  :   [0, 2, 0, 1, 1, 0, 2, 0, 1],
			"value" : [11.2, 6.4, 2.3, 1.1, 10.6, 3.5, 7.2, 2.1, 9.2]
		});

		// zero argmax
		var primer = frame.argmax("value");
		...
		var actual = frame2.argmax("value");
		t.equal(actual, expected);
	});
}
