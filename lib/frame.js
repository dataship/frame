...
function where(selector, condition){
	...
	if(isstring(selector)){
		...
		// virtual column and condition
		condition = selector;
		var keys = this._keys || {};
		// create virtual row
		var row = {};
		var columns = Object.keys(this._cols);
		selector = columns.join("_");
		...
		var value;
		// iterate through length of data set
		for(var i = 0; i < bits.length; i++){
			word = 0|0;
			offset = i * 32;
			for(var j = 31; j >= 0; j--){
				...
				for(var k = 0; k < columns.length; k++){
					column = columns[k];
					value = this._cols[column][index];

					//if(column in keys) row[column] = keys[column][value];
					//else row[column] = value;
					row[column] = value;

				}

				if(condition(row)) word |= 1;
