...
function where(selector, condition){
	...
	for(var i = bits.length - 1; i >= 0; i--){
		...
		if(condition(column[offset])) word |=1;
	...
	*/
	...
	/*
	this._filter = this._filter ? this._filter.and(filter) : filter
	return this;
	*/

	// create and return a new Frame with the new filter
	if(this._filter) filter.and(this._filter);
	return new Frame(this._cols, this._keys, this._index, this._groups, filter);
...
function groupby(){
	...
	return this;
	*/
	return new Frame(this._cols, this._keys, index, selectors.slice(0), this._filter);
};
		...
		for(key in index){ // keys in object
			...
			if(isobject(c)){
				...
				if(filtered.length != 0){
					var value = subsetreduce(column, filtered, reducer, initial);
